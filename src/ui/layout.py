...
def build_category_panel_layout(table_font):
    ...
    return [
        [sg.I(key="_cat_search_", expand_x=True, enable_events=True, size=(18, 1))],
        [
            sg.Table(
...
def build_channel_panel_layout(table_font):
    ...
    return [
        [sg.I(key="_ch_search_", expand_x=True, enable_events=True, size=(28, 1))],
        [
            sg.Table(
                values=[[" "]],
                headings=["Title                                                    ", "Rate", "Year"],
                key="_iptv_content_",
                justification="l",
                expand_y=True,
                expand_x=True,
                ...
                auto_size_columns=False,
                vertical_scroll_only=True,
                col_widths=[35, 6, 6],  # Title gets 40 chars, Rating and Year get 6 chars each
            )
        ],
    ]
...
def build_layout(ui_settings=None):
    ...
    return layout, category_panel_layout, channel_panel_layout  # Return layouts for separate windows
