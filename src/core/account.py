"""Account management dialogs and persistence utilities.

Responsibilities:
- Load/save Xtream account credentials and snapshots
- Provide add and choose account windows
- Refresh server/user snapshots
- Format snapshot info for display

Do NOT perform heavy network operations at import time.
"""
...
def _accounts_load() -> dict:
    """Load all saved accounts from ACCOUNTS_FILE (returns empty dict on failure)."""
    try:
...
def _choose_account_window() -> Optional[AccountChoice]:
    """Show account selection window. Returns tuple ("use", name, account_dict) or None."""
    accs = _accounts_load()
...
def _accounts_save_all(d: dict) -> None:
    """Overwrite ACCOUNTS_FILE with full accounts dictionary."""
    ACCOUNTS_FILE.write_text(json.dumps(d, indent=2), encoding="utf-8")


def _accounts_save_one(name: str, acc: dict) -> None:
    """Merge a single account into accounts file (creates/updates)."""
    d = _accounts_load()
    d[name] = acc
    _accounts_save_all(d)
