...
from services.xtream import _build_m3u_from_xtream  # moved from app
...
def _load_xtream_into_app(
    window: sg.Window, base: str, username: str, password: str, category_window=None
):
    ...
    progress_win = sg.Window(
        "Loading Xtream",
        [[sg.Text("Contacting server...", key="_status_", size=(50,1))], [sg.ProgressBar(100, orientation='h', size=(30, 15), key='_p_')]],
        modal=True,
        keep_on_top=True,
        finalize=True,
        no_titlebar=False,
    ...
    while True:
        ...
        if ev == '_XTREAM_STAGE_':
            ...
            try:
                progress_win['_p_'].update(current_pct:=min(99, pct))
            except Exception:
                pass
    ...
    logger.info("Xtream load complete in %.2fs categories=%d live=%d vod=%d", elapsed, len(Data.categories), len(catalog.get('live', [])), len(catalog.get('vod', [])))
    ...
    def _try_account() -> bool:
        ...
        try:
            ...
            _load_xtream_into_app(
                window, acc["base"], acc["username"], acc["password"], category_window
            )
            return True
        except Exception:
            return False
